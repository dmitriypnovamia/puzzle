...
@interface PatternMatchViewController ()

@property(nonatomic,retain) IBOutlet CheerView *confetiView;


@end
@implementation PatternMatchViewController
...
- (void)viewDidLoad {
    ...
    hint5Price  =   @"Buy 5 Hints";
    hint20Price =   @"Buy 20 Hints";
    hint50Price =   @"Buy 50 Hints";
    proPrice    =   @"Buy Pro";
...
-(void)beginGame{
    ...
    if ([self.stage isEqualToString:@"patterno_New_Born"]){
        levelTypeRndValue = 0;
    }
    else if([self.stage isEqualToString:@"patterno_Playing_Kid"]){
        levelTypeRndValue = 1;
    }
    else if([self.stage isEqualToString:@"patterno_Smarty_Boy"]){
        levelTypeRndValue = 2;
    }
    else if([self.stage isEqualToString:@"patterno_Bachelor"]){
        levelTypeRndValue = 3;
    }
    else if([self.stage isEqualToString:@"patterno_Dad"]){
        levelTypeRndValue = 4;
    }
    else if([self.stage isEqualToString:@"patterno_Grand_Father"]){
        levelTypeRndValue = 5;
    }
    else{
        levelTypeRndValue = 0;
    ...
    if (self.dailyPuzzle == true){
        ...
        levelTypeRndValue = self.dailyPuzzleLevelNo;
    ...
    self.level                  =   [[PatternMatchLevel alloc] initWithFile:@"PatternMatchLevels"];
    ...
    [self.levelNumberLblView startGlowing];
    
    double delayInSeconds      =   2.0;
    dispatch_time_t popTime    =   dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSeconds * NSEC_PER_SEC));
    
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        [self.levelNumberLblView stopGlowing];
    });
}
    ...
    if (toBuy == false) {
        ...
        for(PatternMatchCellLabel *view in _gameLayer.subviews){
            if ([view isKindOfClass:[PatternMatchCellLabel class]]){
                ...
                if(view.cell.type != view.cell.row + 1){
        ...
        if (cellsToReplace.count > 0){
            ...
            for (PatternMatchCellLabel *view in cellsToReplace){
                if(firstViewToMove.cell.type == view.cell.row + 1){
