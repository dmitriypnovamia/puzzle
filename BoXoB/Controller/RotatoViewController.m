...
@interface RotatoViewController ()

@property(nonatomic,retain) IBOutlet CheerView *confetiView;

@end

@implementation RotatoViewController
...
-(void)dummyIncreaseLevel{
    ...
    NSUInteger overAllLevelNumber  =   [[NSUserDefaults standardUserDefaults] integerForKey:@"rotato_levelNumber"];
    
    if (self.levelNumber > levelNumberForStage){

        [[NSUserDefaults standardUserDefaults] setInteger:overAllLevelNumber + 1 forKey:@"rotato_levelNumber"];
...
- (void)addSpritesForCells:(NSMutableArray *)cells {
    ...
    for (Cell *cell in cells) {
        ...
        if (randomIndex == 2){
            textCell = [self addTransformations:textCell];
        }
...
- (void)updateSpritesForCells:(NSMutableSet *)cells withAnimation:(int)animationType{
    for (Cell *cell in cells) {
        CellImage *textCell = [self createCell:cell];
        textCell.currentAngle = cell.currentAngle;
   ...
   // [self addContinueView];
    ...
    for(CellImage *view in _gameLayer.subviews){
        if ([view isKindOfClass:[CellImage class]]){
            ...
            CGFloat angle = cellView.currentAngle;
            if (angle != 0){
                isSuccess = false;
                break;
            }
...
-(IBAction)nintyRight:(UIButton *)btn{
    btn.enabled = false;

    int angle = selectedImage.currentAngle;
    ...
    CGAffineTransform rotate1 = CGAffineTransformMakeRotation(angle*(M_PI/180));
    // Move to the left
    CGRect origionalRect = selectedImage.imgView.frame;
    CGRect rect = CGRectMake((selectedImage.imgView.frame.origin.x + (selectedImage.imgView.frame.size.width/4)), (selectedImage.imgView.frame.origin.x + (selectedImage.imgView.frame.size.width/4)), selectedImage.imgView.frame.size.width/2, selectedImage.imgView.frame.size.width/2);
    // Move to the left
    [UIView animateWithDuration:0.2 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut
                     animations:^(void) {
                         selectedImage.imgView.transform = rotate1;
                         selectedImage.imgView.frame = rect;
                         
                         selectedImage.currentAngle = angle;
                         
                         selectedImage.cell.currentTransform = selectedImage.imgView.transform;
                         selectedImage.cell.currentAngle = selectedImage.currentAngle;
                         [self checkResult];
                     ...
                     completion:^(BOOL b) {
                         ...
                         [UIView animateWithDuration:0.2 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut
                                          animations:^(void) {
                                              selectedImage.imgView.frame = origionalRect;
                                          }
                                          completion:nil];
...
-(IBAction)nintyLeft:(UIButton *)btn{
    btn.enabled = false;
            ...
            if (cellsToReplace.count > 0){
                ...
                [UIView animateWithDuration:0.2 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut
                             animations:^(void) {
                                 ...
                                 firstViewToMove.currentAngle = 0;
                                 
                                 firstViewToMove.imgView.transform = CGAffineTransformIdentity;
                                 
                                 firstViewToMove.cell.currentTransform = firstViewToMove.imgView.transform;
                                 firstViewToMove.cell.currentAngle = firstViewToMove.currentAngle;
                                 firstViewToMove.currentTransform = firstViewToMove.imgView.transform;
                             }
                             completion:^(BOOL b) {
...
-( CellImage *)addTransformations:(CellImage*)view{
    NSUInteger randomIndex = 1 + arc4random() % 3;
    CGAffineTransform rotate = CGAffineTransformMakeRotation(90 * (randomIndex) *(M_PI/180));
        // Move to the left
        // Apply them to a view
    view.currentAngle = 90 * (randomIndex);

    view.imgView.transform = rotate;
    //view.a = -1;
    //view.d = 1;
    
    view.cell.currentTransform = view.imgView.transform;
    view.cell.currentAngle = view.currentAngle;
...
-(void)changeAllCellsFromCell:(CellImage *)img fromDirection:(int)direction isFromUndoRedo:(BOOL)fromUndoRedo{
    if (direction == 0){
        ...
        for (NSInteger column       =   self.level.NumColumns - 1; column > 0; column--) {
            ...
            createdCell.currentAngle = cellToUse.currentAngle;
        ...
        createdCell.currentAngle = lastCell.currentAngle;
        ...
        createdCell.currentAngle = firstCell.currentAngle;
