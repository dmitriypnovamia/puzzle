...
open class TTSegmentedControl: UIView {
    ...
    @IBInspectable open var paddingForView: CGSize = CGSize(width: 30, height: 10)
    @IBInspectable open var cornerRadiusForView: CGFloat = -1 // for rounded corner radius use negative value, 0 to disable
...
extension TTSegmentedControl {
    ...
    fileprivate func configureViewBounds() {
        ...
        frame.size.width = max(frame.size.width, CGFloat(attributedDefaultTitles.count) * (max(unselectedWidth, selectedWidth) + paddingForView.width))
        frame.size.height = max(frame.size.height, max(unselectedHeight + paddingForView.height, selectedHeight + paddingForView.height))
    ...
    fileprivate func updateSelectedViewFrameForLabel(_ label: UILabel) {
        thumbContainerView.frame.size.width = label.frame.size.width + paddingForView.width
    ...
    fileprivate func selectedViewWidthForPoint(_ point: CGPoint)-> CGFloat {
        ...
        var width = label.frame.size.width + paddingForView.height
        if index == 0 {
